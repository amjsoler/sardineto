<?php

namespace Tests\Feature\Clase;

use App\Models\Clase;
use App\Models\Gimnasio;
use App\Models\User;
use Illuminate\Support\Str;
use Illuminate\Testing\Fluent\AssertableJson;
use Tests\TestCase;

class ModificarClaseTest extends TestCase
{
    protected $usuarioInvitado;
    protected $usuarioInvitadoAceptado;
    protected $administrador;
    protected $propietario;

    protected $gimnasio;
    protected $gimnasio2;
    protected $clase;

    protected function setUp(): void
    {
        parent::setUp();

        $this->usuarioInvitado = User::factory()->create();
        $this->usuarioInvitadoAceptado = User::factory()->create();
        $this->administrador = User::factory()->create();
        $this->propietario = User::factory()->create();

        $this->gimnasio = Gimnasio::factory()->create(["propietario" => $this->propietario->id]);
        $this->gimnasio2 = Gimnasio::factory()->create(["propietario" => $this->propietario->id]);

        $this->gimnasio->usuariosInvitados()->attach($this->usuarioInvitado, ["invitacion_aceptada" => false]);
        $this->gimnasio->usuariosInvitados()->attach($this->usuarioInvitadoAceptado);
        $this->gimnasio->administradores()->attach($this->administrador);

        $this->clase = Clase::factory()->create(["gimnasio" => $this->gimnasio->id]);
    }

    public function test_modificar_clase_sin_autenticacion()
    {
        $response = $this->putJson(route("editar-clase",
        [
            "gimnasio" => $this->gimnasio->id,
            "clase" => $this->clase->id
        ]));
        $response->assertStatus(401);
    }
    public function test_modificar_clase_sin_verificar_cuenta()
    {
        $user = User::factory()->create(["email_verified_at" => null]);
        $this->actingAs($user);

        $response = $this->putJson(route("editar-clase",
            [
                "gimnasio" => $this->gimnasio->id,
                "clase" => $this->clase->id
            ]));
        $response->assertStatus(460);
    }
    public function test_modificar_clase_sin_autorizacion()
    {
        $this->actingAs($this->usuarioInvitadoAceptado);
        $response = $this->putJson(route("editar-clase",
            [
                "gimnasio" => $this->gimnasio->id,
                "clase" => $this->clase->id
            ]));
        $response->assertStatus(403);

        $this->actingAs($this->administrador);
        $response = $this->putJson(route("editar-clase",
            [
                "gimnasio" => $this->gimnasio->id,
                "clase" => $this->clase->id
            ]));
        $response->assertStatus(200);

        $this->actingAs($this->propietario);
        $response = $this->putJson(route("editar-clase",
            [
                "gimnasio" => $this->gimnasio->id,
                "clase" => $this->clase->id
            ]));
        $response->assertStatus(200);

        $response = $this->putJson(route("editar-clase",
            [
                "gimnasio" => $this->gimnasio2->id,
                "clase" => $this->clase->id
            ]));
        $response->assertStatus(403);
    }

    public function test_modificar_clase_not_found_route_param()
    {
        $this->actingAs($this->propietario);

        $response = $this->putJson(route("editar-clase",
            [
                "gimnasio" => Gimnasio::orderBy("id", "desc")->first()->id+1,
                "clase" => $this->clase->id
            ]));
        $response->assertStatus(404);

        $response = $this->putJson(route("editar-clase",
            [
                "gimnasio" => $this->gimnasio2->id,
                "clase" => Clase::orderBy("id", "desc")->first()->id+1
            ]));
        $response->assertStatus(404);
    }

    public function test_modificar_clase_validation_fail()
    {
        $this->actingAs($this->propietario);

        //Nombre:max, descripcion:max, fechayhora:date, plazas:integer
        $response = $this->putJson(route("editar-clase", [
            "gimnasio" => $this->gimnasio->id,
            "clase" => $this->clase->id
        ]),
            [
                "nombre" => Str::random(151),
                "descripcion" => Str::random(5001),
                "fechayhora" => "fechainvent",
                "plazas" => 2.1
            ]);
        $response->assertStatus(422);
        $response->assertJson(fn (AssertableJson $json) => $json
            ->has("message")
            ->where("errors.nombre.0", __("validation.clase.nombre.max"))
            ->where("errors.descripcion.0", __("validation.clase.descripcion.max"))
            ->where("errors.fechayhora.0", __("validation.clase.fechayhora.date"))
            ->where("errors.plazas.0", __("validation.clase.plazas.integer"))
        );




        $response = $this->putJson(route("editar-clase", [
            "gimnasio" => $this->gimnasio->id,
            "clase" => $this->clase->id
        ]),
            [
                "plazas" => 0
            ]);
        $response->assertStatus(422);
        $response->assertJson(fn (AssertableJson $json) => $json
            ->has("message")
            ->where("errors.plazas.0", __("validation.clase.plazas.min"))
        );
    }

    public function test_modificar_clase_ok()
    {
        $this->actingAs($this->propietario);

        $response = $this->putJson(route("editar-clase", [
            "gimnasio" => $this->gimnasio->id,
            "clase" => $this->clase->id
        ]),
            [
                "nombre" => "NombreTest",
                "descripcion" => "DescripciónTest",
                "fechayhora" => now()->toDateTimeLocalString(),
                "plazas" => 20
            ]);
        $response->assertStatus(200);
        $response->assertJson(fn (AssertableJson $json) => $json
            ->has("id")
            ->where("nombre", "NombreTest")
            ->where("descripcion", "DescripciónTest")
            ->has("fechayhora")
            ->where("plazas", 20)
            ->where("gimnasio", $this->gimnasio->id)
        );
    }
}
