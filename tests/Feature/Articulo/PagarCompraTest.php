...
class PagarCompraTest extends TestCase
{
    ...
    public function test_pagar_compra_sin_autenticar()
    {
        $response = $this->getJson(route("pagar-compra",
    ...
    public function test_pagar_compra_sin_verificar_cuenta()
    {
    ...
    public function test_pagar_compra_sin_autorizacion()
    {
        //Intentar pagar como usuario normal
    ...
    public function test_not_found_route_params()
    {
        $this->actingAs($this->propietario);
        $response = $this->getJson(route("pagar-compra",
            [
    ...
    public function test_pagar_compra_validation_fail()
    {
        $this->actingAs($this->propietario);

        $this->compra->pagada = now();
        $this->compra->save();
        ...
        $response->assertJson(fn (AssertableJson $json) => $json
            ->has("message")
            ->where("errors.compraId.0", __("validation.articulo.compra.ComprobarSiUnaCompraYaEstaPagada"))
        );
    }

    public function test_pagar_compra_ok()
    {
        $this->actingAs($this->propietario);
        ...
        $this->assertNotNull($this->compra->refresh()->pagada);
    }
}
